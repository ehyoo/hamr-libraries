...
byte Adafruit_L3GD20_Unified::read8(byte reg)
{
  ...
  // while (!Wire.available()); // Wait for data to arrive. //GOLAM: got rid of this to prevent hanging...
  #if ARDUINO >= 100
    value = Wire.read();
  #else
    value = Wire.receive();
...
bool Adafruit_L3GD20_Unified::getEvent(sensors_event_t* event)
{
  ...
  while(!readingValid)
  {
    ...
    raw_gyro.x = (int16_t)(xlo | (xhi << 8));  
    raw_gyro.y = (int16_t)(ylo | (yhi << 8));
    raw_gyro.z = (int16_t)(zlo | (zhi << 8));
